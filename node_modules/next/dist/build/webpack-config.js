...
entry:async()=>{return{...(clientEntries?clientEntries:{}),...entrypoints,...(isServer?{'init-server.js':'next-plugin-loader?middleware=on-init-server!','on-error-server.js':'next-plugin-loader?middleware=on-error-server!'}:{})};},watchOptions:{ignored:['**/.git/**','**/node_modules/**','**/.next/**']},output:{...(isWebpack5?{ecmaVersion:5}:{}),path:outputPath,// On the server we don't use the chunkhash
...
version:`${"9.5.3"}|${nextPublicVariables}|${nextEnvVariables}|${configVars}`,cacheDirectory:_path.default.join(dir,'.next','cache','webpack')};// Adds `next.config.js` as a buildDependency when custom webpack config is provided
...
if(typeof config.webpackDevMiddleware==='function'){const options=config.webpackDevMiddleware({watchOptions:webpackConfig.watchOptions});if(options.watchOptions){webpackConfig.watchOptions=options.watchOptions;}}function canMatchCss(rule){if(!rule){return false;}const fileNames=['/tmp/test.css','/tmp/test.scss','/tmp/test.sass','/tmp/test.less','/tmp/test.styl'];if(rule instanceof RegExp&&fileNames.some(input=>rule.test(input))){return true;}if(typeof rule==='function'){if(fileNames.some(input=>{try{if(rule(input)){return true;}}catch(_){}return false;})){return true;}}if(Array.isArray(rule)&&rule.some(canMatchCss)){return true;}return false;}const hasUserCssConfig=(_webpackConfig$module2=(_webpackConfig$module3=webpackConfig.module)===null||_webpackConfig$module3===void 0?void 0:_webpackConfig$module3.rules.some(rule=>canMatchCss(rule.test)||canMatchCss(rule.include)))!==null&&_webpackConfig$module2!==void 0?_webpackConfig$module2:false;if(hasUserCssConfig){var _webpackConfig$module4,_webpackConfig$plugin,_webpackConfig$optimi,_webpackConfig$optimi2;// only show warning for one build
if(isServer){console.warn(_chalk.default.yellow.bold('Warning: ')+_chalk.default.bold('Built-in CSS support is being disabled due to custom CSS configuration being detected.\n')+'See here for more info: https://err.sh/next.js/built-in-css-disabled\n');}if((_webpackConfig$module4=webpackConfig.module)===null||_webpackConfig$module4===void 0?void 0:_webpackConfig$module4.rules.length){// Remove default CSS Loader
